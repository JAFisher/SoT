    ...
    function parseWorker(lines, currentDir, namespace) {
        for (let i = 0; i < lines.length; i++) {
            ...
            if (methodMatch) {
                ...
                if (!methods[className]) methods[className] = Object.create(null);
...
function generateFiles(baseDir, { nodes, compositionEdges, extendsEdges, methods, types, interfaces, mainBlocks, webBlocks, externals, cliScripts, pkgOverrides, rollups }) {
    ...
    if (rollups && rollups.length > 0) {
        for (const r of rollups) {
            ...
            try {
                console.log("📦 Bundling Rollup into compiled.js for " + (r.namespace || "root") + "...");
                execSync(`npx -y esbuild rollup_entry.ts --bundle --outfile=compiled.js --format=esm`, { cwd: outPath, stdio: "inherit" });
                console.log("✅ Rollup generated: compiled.js");
            } catch (e) {
                console.error("❌ Failed to rollup " + r.namespace + ": " + e.message);
            } finally {
                if (fs.existsSync(rollupEntryPath)) {
                    fs.rmSync(rollupEntryPath);
                }
            }
        }
    }
